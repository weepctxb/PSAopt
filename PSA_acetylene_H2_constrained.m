...
xcoord = linspace(1,16); % Pswing
ycoord = linspace(1,80); % tfeed
zcoord = x(3); % L/D
...
levels=[-0.25,-0.1,-0.03,0,0.1,0.25,0.5,1:5].*1e7;
...
plot(xcoord,adsfrontcon,'r-',desfrontcon,zcoord,'b-',...
    moleboundcon,zcoord,'c-',compresscon,zcoord,'g-.',...
    xcoord,columncon,'m-.');
legend('Lifetime profit/USD','Profit boundary','Optimal point');
...
clearvars xcoord ycoord zcoord fcoord;
xcoord = linspace(9.5,11); % Pswing
ycoord = x(2); % tfeed
zcoord = linspace(9.9,10.1); % L/D
for i=1:length(xcoord) % Pswing
    for j=1:length(zcoord) % L/D
        fcoord(j,i) = objfun([xcoord(i),ycoord,zcoord(j)]);
    end
...
clearvars xcoord ycoord zcoord fcoord;
xcoord = x(1); % Pswing
ycoord = linspace(1,3e3,200); % tfeed
zcoord = linspace(1,12,200); % L/D
...
ylim([1,12]);
levels=[0,3,3.05,3.06,3.07,3.075,3.0775,3.079,3.08,3.0805].*1e5;
...
%% Plot tfeed vs L/D global surface, zoomed in
figure(5); clf; hold on;
clearvars xcoord ycoord zcoord fcoord;
xcoord = x(1); % Pswing
ycoord = linspace(1,100,200); % tfeed
zcoord = linspace(1,18,200); % L/D
for i=1:length(ycoord)
    for j=1:length(zcoord)
        fcoord(j,i) = objfun([xcoord,ycoord(i),zcoord(j)]);
    end
...
%surf(ycoord,zcoord,-fcoord,'EdgeColor','none');
ylim([1,12]);
levels=[-2.5,0,2,2.5,2.8,3.0].*1e5;
contour(ycoord,zcoord,-fcoord,levels,'k--','ShowText','on');
contour(ycoord,zcoord,-fcoord,[0,0],'k-','ShowText','on');
xlabel('t_{feed} / s'), ylabel('L/D'), zlabel('Lifetime profit/USD');
plot([x(2) x(2)],[x(3) x(3)],'k*');
